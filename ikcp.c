...
//
// 上层调用kcp的receive函数，会将rcv_queue中的数据分段整理好填入buffer的用户数据区中，
...
// 
// 在包模式下数据按照用户单次的send数据分界，记录Segment到send_queue中，
// 单次数据量超过mss大小将进行分片处理，
// 分片内的frg记录分片序号，从大到小，0代表本次数据的结束。
// 
// 在流模式下，kcp会将用户的数据全部拼接在一起，
...
static void ikcp_parse_una(ikcpcb *kcp, IUINT32 una)
{
	...
	for (p = kcp->snd_buf.next; p != &kcp->snd_buf; p = next) {
		...
		IKCPSEG *seg = iqueue_entry(p, IKCPSEG, node);
		// 上面的这句展开为 : IKCPSEG *seg = (IKCPSEG*)(   (char*)((IKCPSEG*)p)  - (size_t) &((IKCPSEG *)0)->node   );
...
// 根据接收到的ack遍历snd_buf队列更新各个Segment中ack跳过的次数，
// 用于之后判断是否需要快速重传。
...
// ikcp_input之收包
//
// KCP中，与收包相关联的成员变量有以下几个：
//
// - UInt32 rcv_nxt 下一个要接收的数据包的编号。也就是说此序号之前的包都已经按顺序全部收到了，下面期望收到这个序号的包（已保证数据包的连续性、顺序性）
// - UInt32 rcv_wnd 接收窗口的大小
// - Segment[] rcv_queue 缓存接收到、连续的数据包
// - Segment[] rcv_buf 接收到的数据会先存放到rcv_buf中。 因为数据可能是乱序到达本地的，所以接受到的数据会按sn顺序依次放入到对应的位置中。 
//	 当sn从低到高连续的数据包都收到了，则将这批连续的数据包转移到rcv_queue中。这样就保证了数据包的顺序性。
// - UInt32[] acklist 收到包后要发送的回传确认。 在收到包时先将要回传ack的sn放入此队列中，在flush函数中再发出去。 
//	 acklist中，一个ack以(sn, timestampe)为一组的方式存储。即[{sn1, ts1}, { sn2,ts2 } …] 即[sn1, ts1, sn2, ts2 …]
//
//
//	ikcp_input负责接收用户传入的网络数据，kcp不负责网络端数据的接收，
//	需要用户自己调用相关的网络操作函数进行数据包的接收，将接收到数据通过input传入kcp中。
//	对于用户传入的数据，kcp会先对数据头部进行解包，判断数据包的大小、会话序号等信息，
//	同时更新远端窗口大小。通过调用parse_una来确认远端收到的数据包，
//	将接收到的数据包从snd_buf中移除。然后调用shrink_buf来更新kcp中snd_una信息，
//	用于告诉远端自己已经确认被接收的数据包信息。
//	之后根据不同的数据包cmd类型分别处理对应的数据包 : 
//
//	- IKCP_CMD_ACK  : 对应ack包，kcp通过判断当前接收到ack的时间戳和ack包内存储的发送时间戳来更新rtt和rto的时间。
//	- IKCP_CMD_PUSH : 对应数据包，kcp首先会判断sn号是否超出了当前窗口所能接收的范围，如果超出范围将直接丢弃这个数据包，
//	  如果是已经确认接收过的重复包也直接丢弃，然后将数据转移到新的Segment中，通过parse_data将Segment放入rcv_buf中，
//	  在parse_data中首先会在rcv_buf中遍历一次，判断是否已经接收过这个数据包，如果数据包不存在则添加到rcv_buf中，之后将可用的Segment再转移到rcv_queue中。
//	- IKCP_CMD_WASK : 对应远端的窗口探测包，设置probe标志，在之后发送本地窗口大小。
//	- IKCP_CMD_WINS : 对应远端的窗口更新包，无需做额外的操作。
//
//	然后根据接收到的ack遍历snd_buf队列更新各个Segment中ack跳过的次数，用于之后判断是否需要快速重传。
//	最后进行窗口慢启动的恢复。
//---------------------------------------------------------------------
int ikcp_input(ikcpcb *kcp, const char *data, long size)
{
	...
	while (1) {
		...
		// kcp包头一共24个字节, size减去IKCP_OVERHEAD即24个字节应该应该不小于len
		size -= IKCP_OVERHEAD;
	...
	if (flag != 0) {
		// 根据记录的最大ack的snd值，扫描snd_buff，小于max ack的segment的fastack ++，超过指定阈值，启动快速重传
		ikcp_parse_fastack(kcp, maxack);
	}

	// snd_una与远端una比较，snd_una>una，有未确认的segment，可能存在网络过载, 通过cwnd拥塞窗口控制
...
//	- UInt32 snd_una 当前 未收到确认回传的 发送出去的包的 最小编号。也就是此编号前的包都已经收到确认回传了。
//	- UInt32 snd_nxt 下一个要发送出去的包编号
//	- UInt32 snd_wnd 发送窗口大小
//	- UInt32 rmt_wnd 远端的接收窗口大小
//	- Segment[] snd_queue 发送队列。应用层的数据（在调用KCP.Send后）会进入此队列中，KCP在flush的时候根据发送窗口的大小，再决定将多少个Segment放入到snd_buf中进行发送
//	- Segment[] snd_buf 发送缓存池。发送出去的数据将会呆在这个池子中，等待远端的回传确认，等收到远端确认此包收到后再从snd_buf移出去。 KCP在每次flush的时候都会检查这个缓存池中的每个Segment，如果超时或者判定丢包就会重发。
//
//	Tips :
